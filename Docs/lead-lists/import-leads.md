...
**Acceptance criteria**
- [ ] Given a `leadList` array where each object has at least `email` plus optional `first_name`, `last_name` and `company`, and a `fileName` such as "enterprise-prospects-jan2025.csv", when I import into segment 500, then a 200 returns `total_leads`, `imported`, `duplicates` and `blocked`, and those four numbers add up.
...
- [ ] Given a previously unsubscribed address, when I import, then the lead may join the segment but is permanently excluded from sending, and the row is shown as "unsubscribed — will never be emailed".
