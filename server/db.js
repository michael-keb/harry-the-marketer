...
for (const stmt of [
  ...
  // When this enrolment began. A re-enrolment is a fresh approach and must wait
  // out the person cooling-off like any other; the frequency gate compares this
  // against the campaign's outbound to tell a follow-up from a fresh start. Set
  // by trigger so every insert path — attach, import, handoff, sweep — gets it.
  "ALTER TABLE campaign_leads ADD COLUMN enrolled_at TEXT DEFAULT ''",
  `CREATE TRIGGER IF NOT EXISTS campaign_leads_enrolled_at AFTER INSERT ON campaign_leads
     WHEN NEW.enrolled_at IS NULL OR NEW.enrolled_at = ''
     BEGIN UPDATE campaign_leads SET enrolled_at = datetime('now') WHERE id = NEW.id; END`,
